...
void print_answer(int count_line, int count_char, int len, int oLowerCnt,
                  int oUpperCnt, int oDigitsCnt);
...
void calculate_answer(char *array) {
  ...
  while (1) {
    if (fgets(array, 1000, stdin) != NULL) {
      ...
      print_answer(count_line, count, oLowerCnt + oUpperCnt, oLowerCnt,
                   oUpperCnt, oDigitsCnt);
      count_line += 1;
    } else {
      break;
    }
...
void print_answer(int count_line, int count_char, int len, int oLowerCnt,
                  int oUpperCnt, int oDigitsCnt) {
  printf(
      "Line %d has %d chars: %d are letters (%d lower, %d upper), %d are "
      "digits.\n",
