...
void answer(int *array_x, int *array_y, int count_pair, int *array_value_x,
            int *array_value_y, int *array_answer, int size_R);
...
int main(void) {
  ...
  if (!reading_count(&size_R, &count_pair)) {
    ...
    if (!reading_pair(count_pair, array_x, array_y)) {
      ...
      answer(array_x, array_y, count_pair, array_value_x, array_value_y,
             array_answer, size_R);
      free(array_value_x);
      free(array_value_y);
      free(array_answer);
...
void answer(int *array_x, int *array_y, int count_pair, int *array_value_x,
  ...
  if (array_answer[0] == 1 && array_answer[1] == 1 && array_answer[2] == 1 &&
      array_answer[3] == 1 && array_answer[4] == 1) {
    printf("0");
  } else {
    int mark = 0;
    for (int i = 0; i < 5; i++) {
