...
void reading_numbers_systems(int *error, int *start_number_system,
                             int *end_number_system);
...
void reading_numbers_systems(int *error, int *start_number_system,
                             int *end_number_system) {
  if (scanf("%d %d ", start_number_system, end_number_system) != 2 ||
      *start_number_system < 2 || *end_number_system < 2 ||
      *start_number_system > 36 || *end_number_system > 36) {
    *error = 1;
  }
}
...
int reading_number(int *error, int start_number_system) {
  ...
  while (1) {
    if (scanf("%c", &symbol) == 1 &&
        ((symbol >= 48 && symbol <= 57) || (symbol >= 97 && symbol <= 122))) {
